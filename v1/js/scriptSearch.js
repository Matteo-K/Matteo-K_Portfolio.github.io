  ...
  if (response.ok) {
    response.json().then((file) => {
      for (let index = 0; index < file.length; index++) {
        suggestions.title = file[index].title;
        suggestions.lien = "projet.html?projet=" + file[index].lien;
        suggestions.projet_ = null;
      }
    });
  }
});
...
function linkSelect() {
  ...
  for (let id = 0; id < suggestions.length; id++) {
    if (suggestions[id].title.toUpperCase() === userData.toUpperCase()) {
      page = "projet.html?projet=" + suggestions[id].lien;
